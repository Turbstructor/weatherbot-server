...
def get_weather(refresh):
    '''
        Fetches weather.
        This method 'requests' weather information from openweather API if argument 'refresh' is True; otherwise this method fetches it from the recently saved file.
    '''
    api_key_onecall = ''
    api_key_air = ''
...
def read_weather(refresh, verbose):
    ...
    if refresh:
        with open('.API_KEY', 'r') as in_file:
            api_key_onecall, api_key_air = in_file.read().splitlines()
            print(api_key)

        # url = 'https://api.openweathermap.org/data/2.5/onecall?lat={lat}&lon={lon}&exclude={part}&appid={API key}'
        url = f'https://api.openweathermap.org/data/2.5/onecall?lat={}&lon={}&units=metric&lang=kr&appid={}'.format(lat, lon, api_key)
        ...
        url = f'http://api.openweathermap.org/data/2.5/air_pollution/forecast?lat={}&lon={}&appid={}'.format(lat, lon, api_key)
